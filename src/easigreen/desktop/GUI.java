   ...
   extends Application
{
    ...
    */
   private static final String[] mainNavNames =
      new String[]{ "country", "energy", "upgrades", "goals" };

   /**
    * Holds The Sub Classes of the main navigation panes
    */
   private static final String[][] subNavNames =
      new String[][]
      {
         { "world", "politicalresources", "trade" },
    ...
    * Holds Class Names
    */
   private static final String[] mainClassNames =
      new String[]{ "CountryPane", "EnergyPane", "UpgradesPane", "GoalsPane" };

   /**
    * Holds Holds Sub Class names
    */
   private static final String[][] subClassNames =
      new String[][]
      {
         { "WorldPane", "PoliticalPane", "TradePane" },
         { "NuclearPane", "FossilPane", "RenewablePane", "OilPane" },
         { "SciencePane", "EngineeringPane", "PoliciesPane", "TechnologiesPane" },
   ...
   private void initializeNavigation()
   {
      ...
      for (int mainNavNum = 0; mainNavNum < mainNavNames.length;
             mainNavNum++)
      {
         mainNavigation.addButton(mainNavNames[mainNavNum],
            events.get(mainNavNames[mainNavNum]));

         NavPane newNav = new NavPane();

         for (int subNavNum = 0; subNavNum < subNavNames[mainNavNum].length;
                subNavNum++)
         {
            newNav.addButton(subNavNames[mainNavNum][subNavNum],
               events.get(subNavNames[mainNavNum][subNavNum]));
         }
   ...
   private void initializeContent()
   {
      ...
      try
      {
         for (int navNumber = 0; navNumber < mainNavNames.length;
                navNumber++)
         {
            ...
            Constructor mainConstruct =
               Class.forName(currentClass).getConstructor(SimCity.class);
            mContent.put(mainNavNames[navNumber],
               (ContentPane) mainConstruct.newInstance(mModel));

            for (int subNavNum = 0; subNavNum < subNavNames[navNumber].length;
                   subNavNum++)
            {
               currentClass = packagePrefix +
                  subClassNames[navNumber][subNavNum];

               Constructor subConstruct =
                  Class.forName(currentClass).getConstructor(SimCity.class);
               mContent.put(subNavNames[navNumber][subNavNum],
                  (ContentPane) subConstruct.newInstance(mModel));
            }
         }
      }
      catch (ClassNotFoundException e)
      {
         System.out.println("Fatal Error: " + currentClass +
            " class not found");
         exit();
      }
      catch (InstantiationException e)
      {
         System.out.println("Fatal Error: Could not instantiate " +
            currentClass);
         exit();
      }
      catch (NoSuchMethodException e)
      {
         System.out.println("Fatal Error: Constructor for " + currentClass +
            "Not Found");
         exit();
      }

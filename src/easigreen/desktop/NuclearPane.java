   ...
   extends ContentPane
{
   ...
   protected void update()
   {
      mValues.get("Plants in Operation")
             .setText("" + mModel.getEnergyManager().getNuclear().getAmount());
      mValues.get("Supply Provided")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getPower()));
      mValues.get("Cost")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getCostBuild()));
      mValues.get("Public Approval")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getApproval()));
      mValues.get("Emissions")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getEmissions()));
      mValues.get("Security")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getSecurity()));
      mValues.get("Profit")
             .setText(String.format(FORMAT,
            mModel.getEnergyManager().getNuclear().getProfit()));
   }
   ...
   protected void init()
   {
      ...
      EventHandler<ActionEvent> upgradeEvent =
         new PanelChangeEvent(mNuclearUpgradesPane);
            ...
            public void handle(ActionEvent event)
            {
               ...
               if (current < newAmount)
               {
                  mModel.getResourceManager()
                        .setFunds(mModel.getResourceManager().getFunds() -
               ...
               else if (current > newAmount)
               {
                  mModel.getResourceManager()
                        .setFunds(mModel.getResourceManager().getFunds() -
                     (mModel.getEnergyManager().getNuclear().getCostRemove() * (current -
                     newAmount)));
               }

               mModel.getEnergyManager().getNuclear()
